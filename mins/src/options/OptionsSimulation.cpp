...
void mins::OptionsSimulation::load_print(const std::shared_ptr<ov_core::YamlParser> &parser) {
  if (parser != nullptr) {
    ...
    BSpline_path.substr(0, 13) == "MINS_DATA_DIR" ? BSpline_path.replace(0, 13, ament_index_cpp::get_package_share_directory("mins_data")) : std::string();
    planes_path.substr(0, 13) == "MINS_DATA_DIR" ? planes_path.replace(0, 13, ament_index_cpp::get_package_share_directory("mins_data")) : std::string();
