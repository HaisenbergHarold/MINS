...
void mins::OptionsSystem::load_print(const std::shared_ptr<ov_core::YamlParser> &parser) {
  if (parser != nullptr) {
    ...
    path_timing.substr(0, 8) == "MINS_DIR" ? path_timing.replace(0, 8, ament_index_cpp::get_package_share_directory("mins")) : std::string();
    path_state.substr(0, 8) == "MINS_DIR" ? path_state.replace(0, 8, ament_index_cpp::get_package_share_directory("mins")) : std::string();
    path_trajectory.substr(0, 8) == "MINS_DIR" ? path_trajectory.replace(0, 8, ament_index_cpp::get_package_share_directory("mins")) : std::string();
  }
